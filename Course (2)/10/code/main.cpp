...
int main()
{
	...
	else if (!f.eof())
	{
		...
		do
		{
			...
			cout << "1 - Список студентов без задолжностей " << endl;
			cout << "2 - Процент неуспевающих студентов "<<endl;
			cout << "3 - Выход"<<endl;
			cout << "------------------------------------------------------"<<endl;
			n=cin.get();
			cin.sync();
...
void ne_doljen (int kolvozn, sessia tz[])
{
	int n = 0;
	int i=0;
	cout << "№ Фамилия И.О. зачет экзамен" << endl;
    cout << "----------------------------" << endl;
	while (i != kolvozn)
    {
			if ((tz[i].zachet == "ззззз")&&(tz[i].z1 != "2")&&(tz[i].z2 != "2")&&(tz[i].z3 != "2")&&(tz[i].z4 != "2"))
...
void procent_ne_uspevaushih (int kolvozn, sessia tz[])
{
    ...
    procent = procent*100;
    if (k!=0)
    cout<<"процент неуспевающих = "<<procent<<endl;
    else
    cout<<"Файл пуст"<<endl;
}
